...
type NotificationTypeResponse struct {
	...
	// notification type
	// Enum: ["DEPLOYMENT_STATUS_UPDATE","APP_DEPLOYMENT","QASUITE_SANITY","DR_RESULT","STACK_SIGNOFF","ALERT","TEST","APPLICATION_DEPLOYMENT_COMPLETE","ENVIRONMENT_CREATE","ENVIRONMENT_DELETE","ENVIRONMENT_LAUNCH","ENVIRONMENT_DESTROY","SHORT_LIVED_ENVIRONMENT_TEARDOWN","ENVIRONMENT_CONFIGURE","AUDIT_LOG"]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["DEPLOYMENT_STATUS_UPDATE","APP_DEPLOYMENT","QASUITE_SANITY","DR_RESULT","STACK_SIGNOFF","ALERT","TEST","APPLICATION_DEPLOYMENT_COMPLETE","ENVIRONMENT_CREATE","ENVIRONMENT_DELETE","ENVIRONMENT_LAUNCH","ENVIRONMENT_DESTROY","SHORT_LIVED_ENVIRONMENT_TEARDOWN","ENVIRONMENT_CONFIGURE","AUDIT_LOG"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	// NotificationTypeResponseNotificationTypeSHORTLIVEDENVIRONMENTTEARDOWN captures enum value "SHORT_LIVED_ENVIRONMENT_TEARDOWN"
	NotificationTypeResponseNotificationTypeSHORTLIVEDENVIRONMENTTEARDOWN string = "SHORT_LIVED_ENVIRONMENT_TEARDOWN"
	...
	// NotificationTypeResponseNotificationTypeAUDITLOG captures enum value "AUDIT_LOG"
	NotificationTypeResponseNotificationTypeAUDITLOG string = "AUDIT_LOG"
)
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["CLUSTER_NAME","CLUSTER_TYPE","QASUITE_RESULT","DR_ACTION","DR_STATUS","STACK_NAME","SEVERITY","ALERT_NAME","DEPLOYMENT_STATUS","APPLICATION_NAME","SEND_RESOLVED","RELEASE_TYPE","ENVIRONMENT_TEARDOWN_NOTIFY_BEFORE","AUDIT_ENTITY","AUDIT_ACTION"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
