...
type Port struct {
	...
	// lb port
	LbPort int64 `json:"lbPort,omitempty"`
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["TCP","HTTP","HTTPS"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	// PortProtocolHTTP captures enum value "HTTP"
	PortProtocolHTTP string = "HTTP"

	// PortProtocolHTTPS captures enum value "HTTPS"
	PortProtocolHTTPS string = "HTTPS"
)
